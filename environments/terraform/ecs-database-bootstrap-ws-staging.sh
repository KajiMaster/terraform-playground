...
# ECS Database Bootstrap One-Liner for WS-Staging
# Uses ECS Exec to run database commands from within the ECS task

echo "ECS Database Bootstrap One-Liner for WS-Staging"
echo "==============================================="
echo ""

echo "Getting ECS task ARN..."
TASK_ARN=$(aws ecs list-tasks --cluster ws-staging-ecs-cluster --region us-east-2 --query 'taskArns[0]' --output text)
...
aws ecs execute-command \
    --cluster ws-staging-ecs-cluster \
    --task "$TASK_ARN" \
    --container flask-app \
    --interactive \
...
aws ecs execute-command \
    ...
    --container flask-app \
    --interactive \
    --command "/bin/bash -c \"mysql -h ws-staging-db.c38ukeqk0mqb.us-east-2.rds.amazonaws.com -u tfplayground_user -p$DB_PASSWORD tfplayground_ws_staging < /tmp/bootstrap-contacts.sql\""

echo ""
echo "Database bootstrap complete!"
echo "Check the application at: http://ws-staging-alb-1132437371.us-east-2.elb.amazonaws.com/" 