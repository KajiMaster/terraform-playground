...
# ECS Database Bootstrap One-Liner for Production
# Uses ECS Exec to run database commands from within the ECS task

ENVIRONMENT="production"
REGION="us-east-2"

echo "ECS Database Bootstrap One-Liner for $ENVIRONMENT"
echo "================================================="
echo ""

echo "Getting ECS task ARN..."
TASK_ARN=$(aws ecs list-tasks --cluster ${ENVIRONMENT}-ecs-cluster --region $REGION --query 'taskArns[0]' --output text)
...
DB_PASSWORD=$(aws ssm get-parameter --name "/tf-playground/all/db-pword" --region $REGION --with-decryption --query Parameter.Value --output text)
...
aws ecs execute-command \
    ...
    --container flask-app \
    --interactive \
    --command "/bin/bash -c \"mysql -h ${ENVIRONMENT}-db.c38ukeqk0mqb.us-east-2.rds.amazonaws.com -u tfplayground_user -p$DB_PASSWORD tfplayground_prod < /tmp/bootstrap-contacts.sql\""

echo ""
echo "Database bootstrap complete!"
echo "Check the application at: http://${ENVIRONMENT}-alb-1997691628.us-east-2.elb.amazonaws.com/" 